...
function initEmailGeneratorView() {
  ...
  // --- Download Email as Image ---
  document.getElementById('btn-download-email-image')?.addEventListener('click', () => {
    const subject = document.getElementById('output-email-subject')?.innerText?.trim() || 'Email';
    const body = document.getElementById('output-email-body')?.innerText?.trim() || '';
    if (!body) { showToast('No email content to download.', 'error'); return; }
    downloadTextAsImage({ title: subject, body, filename: 'email' });
  });
}
...
function downloadTextAsImage({ title = '', body = '', filename = 'download' } = {}) {
  ...
  Object.assign(card.style, {
    ...
    boxShadow: '0 8px 48px rgba(0,74,198,.18)',
  ...
  header.innerHTML = `
    <div style="display:flex;align-items:center;gap:10px">
      <div style="width:34px;height:34px;border-radius:10px;background:linear-gradient(135deg,#4f46e5,#0ea5e9);display:flex;align-items:center;justify-content:center">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M20 4H4C2.9 4 2 4.9 2 6v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z" fill="#fff"/></svg>
      </div>
      <span style="font-weight:800;font-size:15px;color:#1e1b4b;letter-spacing:-.3px">fasterway<span style="color:#4f46e5">.ai</span></span>
    </div>
    <span style="font-size:11px;color:#64748b;font-weight:500">AI Generated Email</span>
  `;
  ...
  bodyLabel.textContent = 'EMAIL CONTENT';
  ...
  footer.innerHTML = `
    <span style="font-size:11px;color:#94a3b8">Generated by fasterway.ai &nbsp;·&nbsp; ${new Date().toLocaleDateString('en-US',{year:'numeric',month:'short',day:'numeric'})}</span>
    <span style="font-size:11px;color:#4f46e5;font-weight:600">fasterway.ai</span>
  `;
  ...
  const btn = document.getElementById('btn-download-email-image') || document.getElementById('btn-download-corrected-image');
  const origHTML = btn ? btn.innerHTML : '';
  if (btn) btn.innerHTML = '<span class="material-symbols-outlined text-[15px] animate-spin">progress_activity</span><span>Rendering…</span>';
...
function initGrammarCheckerView() {
  ...
  // --- Download Corrected Text as Image ---
  document.getElementById('btn-download-corrected-image')?.addEventListener('click', () => {
    const corrected = state.currentGrammarResult?.correctedText?.trim() || '';
    if (!corrected) { showToast('No corrected text to download.', 'error'); return; }
    downloadCorrectedAsImage(corrected);
  });
  ...
  function downloadCorrectedAsImage(text) {
    ...
    Object.assign(card.style, {
      ...
      padding: '40px 44px 44px',
      fontFamily: "'Inter', 'Segoe UI', Arial, sans-serif",
      boxShadow: '0 8px 48px rgba(5,150,105,.14)',
      border: '1.5px solid #6ee7b7',
      boxSizing: 'border-box',
    });

    card.innerHTML = `
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:22px;padding-bottom:16px;border-bottom:2px solid #6ee7b7">
        <div style="display:flex;align-items:center;gap:10px">
          <div style="width:34px;height:34px;border-radius:10px;background:linear-gradient(135deg,#059669,#10b981);display:flex;align-items:center;justify-content:center">
          ...
          <span style="font-weight:800;font-size:15px;color:#064e3b;letter-spacing:-.3px">fasterway<span style="color:#059669">.ai</span></span>
        </div>
        <span style="font-size:11px;color:#065f46;font-weight:600;background:#d1fae5;padding:3px 10px;border-radius:999px">✦ AI Corrected Text</span>
      </div>
      <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:#059669;margin-bottom:12px">FULL AI CORRECTED TEXT</div>
      <div style="font-size:13.5px;color:#1a2e1e;line-height:1.75;white-space:pre-wrap;word-break:break-word;background:#fff;border:1px solid #a7f3d0;border-radius:12px;padding:16px 18px">${text.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</div>
      <div style="margin-top:24px;padding-top:14px;border-top:1px solid #a7f3d0;display:flex;justify-content:space-between;align-items:center">
        <span style="font-size:11px;color:#6b7280">Grammar-corrected by fasterway.ai &nbsp;·&nbsp; ${new Date().toLocaleDateString('en-US',{year:'numeric',month:'short',day:'numeric'})}</span>
        <span style="font-size:11px;color:#059669;font-weight:600">fasterway.ai</span>
      </div>
    `;
