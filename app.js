/**
 * app.js - growfasting Real-Time AI Email Generator & Grammar Checker Engine
...
/* =====================================================
   ...
   ===================================================== */
function initTheme() {
  const savedTheme = localStorage.getItem('growfasting_theme') || 'system';
  applyTheme(savedTheme);

  // Listen for OS system theme updates
  window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', () => {
    const current = localStorage.getItem('growfasting_theme') || 'system';
    if (current === 'system') {
      applyTheme('system');
    }
  });
...
function applyTheme(theme) {
  localStorage.setItem('growfasting_theme', theme);
...
function initNavigation() {
  ...
  navLinks.forEach(link => {
    ...
    link.addEventListener('click', (e) => {
      ...
      const path = link.getAttribute('data-path');
      if (path === 'sign-in') {
        showToast('Authentication is managed via growfasting user accounts.', 'info');
        return;
      }
      switchView(path);
    });
...
async function generateEmail() {
  ...
  outputPlaceholder.innerHTML = `
    <div class="flex flex-col items-center justify-center p-12 text-center ai-loading-pulse">
      ...
      <h3 class="text-headline-sm font-bold text-on-surface mb-2">growfasting Real-Time AI is writing your email</h3>
  ...
  try {
    ...
    showToast('Email successfully generated by growfasting AI!', 'success');
